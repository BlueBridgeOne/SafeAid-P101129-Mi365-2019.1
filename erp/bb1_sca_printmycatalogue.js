			...
			if (customer) {
				...
				var itemSearchObj = search.create({
					...
					columns: [
						...
						search.createColumn({
							name: "custitem_bb1_matrix_shoewidths",
							label: "Shoe Widths"
						}),
						search.createColumn({
							name: "custitem_bb1_matrix_ladieswear",
							label: "Ladies Wear Sizes"
						}),
						search.createColumn({
				...
				itemSearchObj.run().each(function (result) {
					item = {
						...
						parentDisplayName: result.getValue({
							name: "displayname",
							join: "parent"
						}) || result.getValue({
							name: "description",
					...
					item.name = xmlSafe( item.parentDisplayName || item.displayname || item.salesdescription);
				...
				items.sort(function (a, b) {
					return a.salesdescription.localeCompare(b.salesdescription)
				});
