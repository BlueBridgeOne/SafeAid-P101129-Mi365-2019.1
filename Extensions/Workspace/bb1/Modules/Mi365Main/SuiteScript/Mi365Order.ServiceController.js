		...
		return ServiceController.extend({
			...
			get: function get() {
				...
				if (task == "approve") {
					...
					var purchasenumber = request.getParameter("purchasenumber");
					if(purchasenumber=="ThrowError"){
						throw(new Error("This is a test."));
					}
					var rec = nlapiLoadRecord(this.recordtype, id);
					var numLines = rec.getLineItemCount('item'),plannedShipDate;
					
					for (var i = 1; i <= numLines; i++) {
						plannedShipDate=rec.getLineItemValue('item','custcol_bb1_plannedshipdate',i);
						if(!plannedShipDate){
							rec.setLineItemValue('item','custcol_bb1_plannedshipdate',i, new Date());
						}
						
					}
					rec.setFieldValue("otherrefnum", purchasenumber); 
