					...
					if (contactSearch) {
						for (var i = 0; i < contactSearch.length; i++) {
							...
							if (id) { //add choices
								...
								data.custentity_bb1_sca_duration.choice = [{
									value: "1",
									text: "Per Month"
								}, {
									value: "2",
				...
				for (var j = 0; j < this.fields.length; j++) {
					...
					if (value && !this.fields[j].listonly) {
						try {
							...
							} else if (this.fields[j].type == "multichoice") {
								//nlapiLogExecution("debug", "multichoice", JSON.stringify(this.data[this.fields[j].id].split(',')));
								rec.setFieldValue(this.fields[j].id, value.split(','));
							...
							nlapiLogExecution("debug", "update buyers", "unable to set " + this.fields[j].id + " to " + value + ". " + e);
						}
					}
				}
				nlapiSubmitRecord(rec, true, true);
