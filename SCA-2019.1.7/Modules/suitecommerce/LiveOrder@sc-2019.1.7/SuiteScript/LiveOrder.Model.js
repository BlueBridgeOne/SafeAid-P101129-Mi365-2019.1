...
define(
	'LiveOrder.Model', [
		'SC.Model', 'Application', 'Profile.Model', 'StoreItem.Model', 'SC.Models.Init', 'SiteSettings.Model', 'Utils', 'ExternalPayment.Model', 'underscore', 'CustomFields.Utils', 'Configuration'
	],
	function (
		SCModel, Application, Profile, StoreItem, ModelsInit, SiteSettings, Utils, ExternalPayment, _, CustomFieldsUtils, Configuration
	) {
		'use strict';
		...
		return SCModel.extend({
			...
			addLines: function addLines(lines_data) {
					...
					var lines_ids = ModelsInit.order.addItems(items),
						...
						order_item_ids = _.pluck(lines_ids, 'orderitemid')
						// Stores the current order
						,
			...
			getAutomaticallyRemovedPromocodes: function getAutomaticallyRemovedPromocodes(current_order) {
					var latest_promocodes = _.pluck(_.where(ModelsInit.order.getFieldValues(['promocodes']).promocodes, {
						...
						current_promocodes = _.pluck(_.where(current_order.promocodes, {
							isvalid: true
						}), 'code'),
			...
			getPaymentMethods: function getPaymentMethods(order_fields) {
					var paymentmethods = [],
						...
						paypal = payment && _.findWhere(ModelsInit.session.getPaymentMethods(), {
							ispaypal: 'T'
						}),
					...
					if (credit_card && credit_card.paymentmethod && credit_card.paymentmethod.creditcard === 'T') {
						...
						var customer_invoice = ModelsInit.customer.getFieldValues([
							'paymentterms', 'creditlimit', 'balance', 'creditholdoverride'
						]);
